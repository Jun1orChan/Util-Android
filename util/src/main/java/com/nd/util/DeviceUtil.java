...
public class DeviceUtil {
    ...
    private static Pattern sPattern = Pattern.compile("(\\d+\\.){2}\\d");
...
//            Log.e("TAG", "read SystemProperties error:" + e.getLocalizedMessage());
        }
    }
    ...
    public static boolean isFlymeLowerThan(int majorVersion, int minorVersion, int patchVersion) {
        ...
        if (sFlymeVersionName != null && !sFlymeVersionName.equals("")) {
            try {
                Matcher matcher = sPattern.matcher(sFlymeVersionName);
                if (matcher.find()) {
